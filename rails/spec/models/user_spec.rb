  ...
  describe "from_omniauth" do
    ...
    describe "when the user does not exist" do
      it "creates the user" do
        expect {
          User.find_or_create_from_omniauth(google_oauth_provider)
        }.to change { User.count }.by(1)
      end

      it "correctly initializes the user" do
        created_user = User.find_or_create_from_omniauth(google_oauth_provider)
    ...
    describe "when the user already exists" do
      let!(:existing_user) { User.find_or_create_from_omniauth(google_oauth_provider) }

      it "returns the existing user" do
        expect(User.find_or_create_from_omniauth(auth_hash).id). to eq(existing_user.id)
      end
